function printEvenOddNamesAsync() {
  const names = ["Ali", "Sara", "Ahmed", "Muqeem", "Noor", "Rasid", "Aisha", "Omar"];

  const evenNames = []; // full names at even indexes
  const oddNames = [];  // full names at odd indexes
  let i = 0;

  // Clear old output
  document.getElementById('evenOutputAsyncNames').textContent = '';
  document.getElementById('oddOutputAsyncNames').textContent = '';
    ...
    if (i < names.length) {
      setTimeout(function() {
        if (i % 2 === 0) {
          ...
          document.getElementById('evenOutputAsyncNames').textContent = evenNames.join(', ');
        } else {
          oddNames.push(names[i]);
          document.getElementById('oddOutputAsyncNames').textContent = oddNames.join(', ');
        }

        i++;
        processNext();
      }, 300); // delay to show async effect
    }
  }
